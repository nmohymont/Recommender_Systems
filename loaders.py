...
def load_tmdb_features():
    """
    Load TMDB features (poster_url, overview, runtime, vote_average, popularity).
    Returns a DataFrame indexed by movieId.

    Colonnes utiles :
        - overview    : description textuelle du film
        - poster_url  : URL du poster TMDB
        - runtime     : durée en minutes
        - vote_average: note moyenne TMDB
        - vote_count  : nombre de votes TMDB
    ...
    df["movieId"] = df["movieId"].astype(float).astype(int)
    df = df.set_index("movieId")

    # Nettoyer les overviews vides
